...
const Profile = ({ user, setUser }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Even if backend fails, we should clear local storage for security
      localStorage.removeItem("user");
      setUser(null);
      navigate("/login");
    }
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white/60 text-center">
              <div className="relative inline-block mb-6">
                <div className="w-32 h-32 bg-gradient-to-tr from-slate-900 to-indigo-900 rounded-[2.5rem] p-1 shadow-2xl rotate-3">
                  <div className="w-full h-full bg-slate-950 rounded-[2.2rem] flex items-center justify-center text-white text-5xl font-black -rotate-3 italic border border-white/10">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white">
              ...
              <div className="mt-10 grid grid-cols-2 gap-4">
                <button className="py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200">
                  Edit Profile
                </button>
                <button className="py-4 bg-slate-950 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
                  Order History
                </button>
              </div>
            </div>
